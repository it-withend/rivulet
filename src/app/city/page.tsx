...
import { getCityReport } from "@/lib/city-report";
import { CITIES, DEFAULT_CITY } from "@/lib/cities";
import { ONE_HEALTH_PARAMETERS } from "@/lib/science/one-health";
...
export default async function CityReportPage(props: PageProps<"/city">) {
  ...
  const report = await getCityReport(city);
  const {
    streamCount, ratedCount, recentCount, activePeople, byClass, unrated, takeCare, needsVisit, nearChildren, hasSynthetic,
  } = report;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10 px-4 py-10 sm:px-6">
      ...
      <section aria-label="At a glance" className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat icon={<Waves />} value={String(streamCount)} label="stream sections mapped" />
        <Stat
          icon={<SearchCheck />}
          value={streamCount ? `${Math.round((ratedCount / streamCount) * 100)}%` : "—"}
          label="have enough reports to rate"
        />
        <Stat icon={<CalendarDays />} value={String(recentCount)} label={`reports in the last ${ONE_HEALTH_PARAMETERS.windowDays} days`} />
        ...
        {streamCount === 0 ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-river/40 bg-paper-raised p-4">
            <p className="m-0 flex items-center gap-2 text-sm">
              ...
              {city} was just added to Rivulet: {streamCount} stream sections are
          ...
          <>
            <div className="flex h-6 w-full overflow-hidden rounded-sm border border-rule" role="img"
              ...
              {byClass.map((c) =>
                c.count > 0 ? (
                  <div key={c.klass} style={{ width: `${(c.count / streamCount) * 100}%`, backgroundColor: colourForClass(c.klass) }} />
                ) : null,
              )}
              {unrated > 0 && (
                <div className="hatch-insufficient" style={{ width: `${(unrated / streamCount) * 100}%` }} />
              )}
            </div>
              ...
              <li key={s.id}>
                <Link href={`/water/${s.id}`} className="block h-full rounded-md border border-rule bg-paper-raised p-4 text-ink no-underline hover:border-ink">
                  ...
                  <p className="m-0 mt-1 flex items-center gap-1.5 text-sm">
                    <span aria-hidden="true" className="inline-block size-3 rounded-full" style={{ backgroundColor: CONCERN_COLOUR[s.concern] }} />
                    {CONCERN_LABEL[s.concern]}
                  </p>
                  <p className="m-0 mt-1 text-sm text-ink-muted">Reported: {s.signs.join(", ")}</p>
                  {s.place && (
                    <p className="m-0 mt-1 flex items-center gap-1 text-sm text-ink-muted">
                      <MapPin aria-hidden="true" className="size-3.5" /> {s.place}
                    </p>
                  )}
                </Link>
              </li>
            ...
            {needsVisit.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 rounded-md border border-rule bg-paper-raised p-4">
                <div className="min-w-0">
                  ...
                  {s.place && (
                    <p className="m-0 mt-1 flex items-center gap-1 text-sm text-ink-muted">
                      <MapPin aria-hidden="true" className="size-3.5 shrink-0" /> {s.place}
                    </p>
                  )}
                </div>
